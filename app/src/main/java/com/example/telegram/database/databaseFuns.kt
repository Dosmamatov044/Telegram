...
fun sendMessage(message: String, receivingUserID: String, typeText: String, function: () -> Unit) {
 ...
 fun sendMessageFromGroup(message: String, Groupid: String, typeText: String, function: () -> Unit) {
   // val refDialogUser = "$NODE_MESSAGES/$CURRENT_UID/$receivingUserID"
   // val refDialogReceivingUser = "$NODE_MESSAGES/$receivingUserID/$CURRENT_UID"

  val groupMessage="$NODE_GROUPS/$Groupid/$NODE_MESSAGES"

    val messageKey = REF_DATABASE_ROOT.child(groupMessage).push().key
    ...
    mapMessage[CHILD_TIMESTAMP] =
         ...
         REF_DATABASE_ROOT.child(groupMessage)
...
fun sendMessageAsFile(
    ...
    val refGroupFile = "$NODE_GROUPS/$id/$NODE_MESSAGES"
    val key= REF_DATABASE_ROOT.child(refGroupFile).push().key

    val mapMessage = hashMapOf<String, Any>()
    mapMessage[CHILD_FROM] = CURRENT_UID
    mapMessage[CHILD_TYPE] = typeMessage
    mapMessage[CHILD_ID] = key.toString()
    ...
    REF_DATABASE_ROOT.child(refGroupFile).child(key.toString())
            .updateChildren(mapMessage)
            .addOnFailureListener { showToast(it.message.toString()) }
}
...
fun sendMessageAsFileToGroup(
    ...
    val groupMessage="$NODE_GROUPS/$receivingUserID/$NODE_MESSAGES"

    val messageKey1 = REF_DATABASE_ROOT.child(groupMessage).push().key

    val mapMessage = hashMapOf<String, Any>()
    mapMessage[CHILD_FROM] = CURRENT_UID
    mapMessage[CHILD_TYPE] = typeMessage
    mapMessage[CHILD_ID] = messageKey1.toString()
    mapMessage[CHILD_TIMESTAMP] = ServerValue.TIMESTAMP
    mapMessage[CHILD_FILE_URL] = fileUrl
    mapMessage[CHILD_TEXT] = filename


    REF_DATABASE_ROOT.child(groupMessage).child(messageKey1.toString())
            .updateChildren(mapMessage)
...
fun uploadVideoToStorage(uri: Uri,
                                 messageKey:
                                 String, id:
                                 String,
                                 typeMessage:
...
fun sendMessageToHistory(messageKey: String, videoUrl: String, messageKey1: String, typeMessage: String, filename: String)
{
        val groupMessage="$NODE_HISTORY/$messageKey/$NODE_MESSAGES"

        val messageKey2 = REF_DATABASE_ROOT.child(groupMessage).push().key

        val mapMessage = hashMapOf<String, Any>()
        mapMessage[CHILD_FROM] = CURRENT_UID
        mapMessage[CHILD_TYPE] = typeMessage
        mapMessage[CHILD_ID] = messageKey2.toString()
        mapMessage[CHILD_TIMESTAMP] = ServerValue.TIMESTAMP
        mapMessage[CHILD_VIDEO_URL] = videoUrl
        mapMessage[CHILD_TEXT] = filename
